...
CLCD36HF1 <- readRDS(file = "../results/CLCD36HF1/CLCD36HF1_Seuratv4.rds")
CLCD36HF2 <- readRDS(file = "../results/CLCD36HF2/CLCD36HF2_Seuratv4.rds")
CLWTHF1 <- readRDS(file = "../results/CLWTHF1/CLWTHF1_Seuratv4.rds")
CLWTHF2 <- readRDS(file = "../results/CLWTHF2/CLWTHF2_Seuratv4.rds")

# 1.2 Add extra column to the Seurat objects with the condition

CLWTHF1 <- AddMetaData(object = CLWTHF1, metadata = "Control", col.name = "Condition")
CLWTHF2 <- AddMetaData(object = CLWTHF2, metadata = "Control", col.name = "Condition")
CLCD36HF1 <- AddMetaData(object = CLCD36HF1, metadata = "KO", col.name = "Condition")
CLCD36HF2 <- AddMetaData(object = CLCD36HF2, metadata = "KO", col.name = "Condition")

# 1.3 Merge Seurat objects

merged_seurat <- merge(x = CLWTHF1, y = c(CLWTHF2, CLCD36HF1, CLCD36HF2), 
                       add.cell.ids = c("CLWTHF1", "CLWTHF2","CLCD36HF1","CLCD36HF2"))
merged_seurat
...
rm(CLCD36HF1,CLCD36HF2, CLWTHF1, CLWTHF2)
...
data_combined <- FindClusters(data_combined, resolution = 1, cluster.name = "unintegrated_clusters")
...
data_combined <- FindClusters(data_combined, resolution = 1)
...
## Combined by cluster
DimPlot(data_combined, reduction = "umap", label = TRUE, pt.size = 1, label.size = 6)
...
DimPlot(data_combined, reduction = "tsne", label = TRUE, pt.size = 1,label.size = 9)
ggsave(filename = "tsne_clusters.png", path = "../results/Integrated_Analysis/", width = 13, height = 8,
       units = "in", dpi = 300)
...
clusters_order <- c("B cells","T cells","NK cells", "NKT cells","Kupffer cells","Caps","LAMs","Monocytes + cDCs","Granulocytes","pDCs","Hepatocytes")
...
# Variable marcadores contain the marker (differentially expressed genes) for each of the identity classes in a dataset
marcadores <- FindAllMarkers(data_combined, min.pct = 0.25, logfc.threshold = 1)

# We select only top 10 markers for each cell type
...
write.table(x = marcadores,file = "../results/Integrated_Analysis/Marcadores_clusters_IDs.txt")
...
markers_to_plot <- c("Cd19","Cd79a","Cd3g","Trac","Ccl5","Ctla4", "Gzmk","Adgre1","Clec4f","Timd4",
                     "Ccr2","Cx3cr1","Itgam","Dpp4", "Cxcr2","Hdc","Siglech","Nr1h3","Maf","Alb","Serpina3k")
...
## Specif-markers variable contain selected markers
specif_markers = c("Cd79a","Cd3g","Ccl5","Ctla4","Clec4f","Adgre1",
               "Cx3cr1", "Trem2", "Dpp4", "Hdc", "Siglech", "Alb")
VlnPlot(object = data_combined, features = specif_markers, pt.size = 0, ncol = 1,same.y.lims = T)& 
...
# Here we see how does cluster membership vary by condition
...
rownames(cell_percentages_table) <- c(rownames(cell_percentages_table[-12,]),"Total")
...
# Here we see how does cluster membership vary by sample
...
colnames(cell_percentages_table_sample) <- c("CLWTHF1_(%)","CLWTHF2_(%)","CLCD36HF1_(%)","CLCD36HF2_(%)",
                                             "CLWTHF1","CLWTHF2","CLCD36HF1","CLCD36HF2","Total_cells")
...
rownames(cell_percentages_table_sample) <- c(rownames(cell_percentages_table_sample[-12,]),"Total")
...
color_list <- ggplotColours(n=11) ## Asign as many colors as clusters
...
bplot <- barplot(conditions_cluster_perc,  legend.text = TRUE, col = color_list_0.4,border = color_list, beside = TRUE, 
                 ...
                 main = "Cell proportions", sub = "Split by condition", ylim = c(0,50),
...
bplot <- barplot(conditions_cluster_perc_sample,  legend.text = TRUE, col = color_list_0.4,border = color_list, beside = TRUE, 
                 ylab = quote(bold("Percentage (%)")), 
                 main = "Cell proportions", sub = "Split by sample", ylim = c(0,55),
                 args.legend = list(x="topright", inset =c(-0.05,0),cex = 0.8, bty="n"))
...
# Select the different cell types we aerr using in our analysis
...
for (x in 1:length(clusters_KCs)) {
  ...
  # Assign DEGs colors according to their FC and p-value
  keyvals <- ifelse(
...
##################################################
## 8) Analysis of hepatic macrophage population ##
##################################################

## Subset macrophages
Macro <- subset(data_combined, idents = c("Kupffer cells", "LAMs", "Caps"))
# Visualization of this subset of cells and its expression of KC specific markers
DimPlot(Macro, split.by = "Condition")
FeaturePlot(Macro, split.by = "Condition", features = c("Timd4","Clec4f"))

##############################
## 9) Automated annnotation ##
##############################
...
ggsave("../results/Integrated_Analysis/Automated_aannotation.png", width = 11, height = 9, dpi = 600, units = "in")
