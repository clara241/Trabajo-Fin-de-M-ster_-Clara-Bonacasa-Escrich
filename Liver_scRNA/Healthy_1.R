...
#####################################################################
## 1) Reading human liver scRNA-seq dataset from patient Healthy 1 ##
#####################################################################

## Loading data from Healthy 1 which is a single file. The data 
## is available in the GEO database: GSE136103.

sc <- Read10X("../data/Healthy_1/")
...
seurat_object <- CreateSeuratObject(counts = sc, project = "Healthy_1", min.cells = 10, min.features = 200)
dim(seurat_object)
# [1] 12363  1860
...
plot1 + plot2
ggsave(path = "../results/Healthy_1", filename = "Healthy_1_QC_Feature_plot.png", height = 10, width = 13, units = "in", dpi = 300)

## We can hence remove those cells with lower than 200 genes, higher than some total of 2200 UMIs and 
...
seurat_object <- subset(seurat_object, subset = nFeature_RNA > 200 & nFeature_RNA < 2200 & percent.mt < 10)

#We can see the reduction in dimension after sub-setting
dim(seurat_object)
## [1] 12363  1736

saveRDS(object = seurat_object,file = '../results/Healthy_1/Healthy_1_data_combined.rds')
